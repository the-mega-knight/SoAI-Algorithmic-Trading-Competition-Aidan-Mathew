...
#
# FIX (Aug 2026): these were previously hardcoded to datetime(2026, 8, 1) /
# datetime(2026, 8, 29) to mirror the official competition trading window.
# That silently *clipped* local dev backtests down to whatever slice of the
# synthetic CSV data (generated by scripts/generate_synthetic_data.py,
# which stamps roughly "today minus 60 days" through "today") happened to
# overlap that fixed range -- as little as ~4 days depending on when the
# data was generated. A 4-day window is nowhere near enough history for a
# 20-day SMA to ever produce a real signal, and combined with a data/signal
# bug this produced a 0-trade backtest that looked like a clean run.
#
# Left as None so local backtests always use the full available range of
# whatever's actually in data/ -- check the printed "[INFO] Backtest
# window: ..." line at the top of a run to see exactly what range was used.
# Re-introduce an explicit window only if you deliberately want to test a
# specific slice, and make sure it's inside the range your CSVs cover.
